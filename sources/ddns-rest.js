...
exports.create = function(opts) {
    ...
    opts.store = opts.store || require('./ddns-rest-store.js').create({
    ...
    return PromiseA.resolve(opts.store).then(function(DnsStore) {
        ...
        function ddnsTokenWall(req, res, next) {
            ...
            if (!domains.every(function(entry) {
                ...
                res.send({
                    error: {
                        message: "Not authenticated for '" + (err && err.message || err) + "'"
                    }
                });
                return;
            }
        ...
        */
        ...
        function ddnsUpdater(req, res) {
            ...
            if (!Array.isArray(updates)) {
                res.send({
                    error: {
                        message: 'usage: POST [{ "name": "example.com", "value": "127.0.0.1", "ttl": 300, "type": "A" }]'
                    }
                });
                return;
            }
        ...
        function ddnsUpdaterNew(req, res, updates) {
            ...
            if (!updates.every(function(update) {
                    ...
                    if (!ndns.consts.NAME_TO_QTYPE[update.type.toString().toUpperCase()]) {
                        err = {
                            error: {
                                message: "unrecognized type '" + update.type + "'"
                            }
                        };
                        return false;
                    }
                    ...
                    if (!update.value) {
                        err = {
                            error: {
                                message: "missing key (hostname) and or value (ip)"
                            }
                        };
                        return false;
                    }
                ...
                promise = promise.then(function() {
                    ...
                    if (domain.destroy) {
                        return DnsStore.Domains.destroy(domain.id).then(function() {
                            ...
                            updates[i] = {
                                error: {
                                    message: "db error for destroy '" + domain.name + "'"
                                }
                            };
                        });
                    }
                    ...
                    return DnsStore.Domains.upsert(domain.id, domain).then(function() {
                        ...
                        updates[i] = {
                            error: {
                                message: "db error for '" + domain.name + "'"
                            }
                        };
                    });
                }).then(function() {
                    ...
                    updates[i] = {
                        error: {
                            message: (err.message || err.code || err.toString().split('\n')[0])
                        }
                    };
                });
            });
        ...
        function ddnsUpdaterOld(req, res, updates) {
            ...
            domains.forEach(function(domain, i) {
                promise = promise.then(function() {
                    ...
                    return DnsStore.Domains.find({
                        zone: domain.zone
                    }).then(function(existingDomains) {
                        var registered = existingDomains.some(function(d) {
                            return d.registered;
                        });
                        var oldDomains = existingDomains.filter(function(d) {
                            return (domain.host === d.name || domain.host === d.zone) && domain.type === d.type;
                        });
                        var p2;
                        var hostname;
        ...
        */
        ...
        Devices.restful.update = function(req, res) {
            ...
            Devices.update(token, dev).then(function() {
                ...
                res.send({
                    error: {
                        message: 'INTERNAL ERROR (not your fault)'
                    }
                });
            });
        };
        ...
        app.get(apiBase + '/records', logr, expressJwt({
            secret: pubPem
        }), Records.restful.get);
        app.post(apiBase + '/records', logr, expressJwt({
            secret: pubPem
        }), Records.restful.update);
        app.delete(apiBase + '/records/:name/:type/:value/:device?', logr, expressJwt({
            secret: pubPem
        }), Records.restful.destroy);

        app.post(apiBase + '/devices', logr, expressJwt({
            secret: pubPem
        }), Devices.restful.update);
        app.delete(apiBase + '/devices/:name', logr, expressJwt({
            secret: pubPem
        }), Devices.restful.destroy);
        app.delete(apiBase + '/devices/:name/:tld/:sld/:sub?', logr, expressJwt({
            secret: pubPem
        }), Devices.restful.detach);
        app.delete(apiBase + '/devices/:device/:domain', logr, expressJwt({
            secret: pubPem
        }), Devices.restful.detach);

        app.post(apiBase + '/dns/', logr, expressJwt({
            secret: pubPem
        }), ddnsTokenWall, ddnsUpdater);
        app.post(apiBase + '/ddns', logr, expressJwt({
            secret: pubPem
        }), ddnsTokenWall, ddnsUpdater);
